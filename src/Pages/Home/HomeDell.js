...
const HomeDell = () => {
  const { data: dellProducts = [] } = useQuery({
    queryKey: ["dellProducts"],
    queryFn: () =>
      fetch(`${process.env.REACT_APP_PORT}/products?category=dell`).then(
        (res) => res.json()
      ),
  });
  return (
    <>
      <div className="mt-5 pt-16 rounded-md">
        <div className="mb-5">
          <h2 className="text-5xl py-2 text-center font-bold">
            Dell: {dellProducts?.length}
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 px-5 pb-10">
          {dellProducts.map((dell) => (
            <Product dell={dell} key={dell?._id}></Product>
          ))}
        </div>
